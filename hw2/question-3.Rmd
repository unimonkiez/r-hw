...
1. Can you perform a T-test? If you can, perform a T-test and explain the results. If the answer is no, explain why not. Remember the T-test assumption (random sampling, normality of data distribution, ….) (R code)
    ```{r}
    t.test(Groap_A, Groap_B);
    ```
    The test can be coducted becuase  
    * Unknown deviation. 
    * Compared with another group.  
    * Non paired.  
    * Unknown if same variance.  

    But there were few assumptions made: 
    * Both group are sampled from bigger groups which are normaly distributed.  
