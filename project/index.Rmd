...
The app have been evolved quite a lot since the beginning, it started as a simple timer for a specific exercise climbers do, and today is a social media platform for working out.  
...
Through the migration something was missing.  
...
# Tests
## Manipulate the data
Firstly, we will change add some columns to the dataframe to suit our tests better.  
Currently we have Average exp for each attribute, now will will merge those to single "average relevant exp".  
```{r}
...
tree = rpart(difficulty ~ duration + Average.total.exp + Average.relevant.exp, data = train, method = 'anova', minsplit=2, minbucket=5);
rpart.plot(tree);
...
Since the user picks which type of exercise it is before publishing the exercise, we can split the split the data to different types and develop an algorithm for each type.  
After the first user completes the exercise, the difficulty will be decided by the time it took him (duration), his total experience and relevant experience.  
As for the algorithms themselves, we chose to use multiple linear regression test as it's very simple to implement later in the app.  
...
> since it has R2 of `r summary(mlrTimeModel)$r.squared` and the others are - `r summary(mlrTimeModel21)$r.squared`, `r summary(mlrTimeModel22)$r.squared`, `r summary(mlrTimeModel23)$r.squared`, `r summary(mlrTimeModel11)$r.squared`, `r summary(mlrTimeModel12)$r.squared`, `r summary(mlrTimeModel13)$r.squared`.   
...
# Implementation
Now once we have those equations for each exercise type, usage is pretty easy.  
...
The code takes the difficulty set for the exercise, multiples it with a predefined factor which converts the scale of the difficulty (1-5) to experience (0-10k for now) 
and splits it evenly (that's why there is division) between each muscle the exercise is set to workout on.  
...
We should mention that the predictors aren't hardcoded to the server, and rerunning this R code and updating the values inside the configuration table (used by ConfigurationRepository) is enough to update the model's equation.  
And Voila! Now even new exercises rewards the users with experience!  
